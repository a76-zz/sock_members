...
define({
  __extend: function(state, target) {
    var that = this;
    var state = state || { buffer: [], map: [] };
    var target = target || {};

    target.count = function() {
      return that.count(state);
    };

    target.contains = function(range) {
      return that.contains(state, range);
    };

    target.write = function(data, range) {
      return that.write(state, data, range);
    };

    target.read = function(range) {
      return that.read(state, range);
    };

    target.read_unsafe = function(range) {
      return that.read_unsafe(state, range);
    };

    target.coverage = function(range, capacity, count) {
      return that.coverage(state, range, capacity, count);
    };
